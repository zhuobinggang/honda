from roberta import encode_title_append, roberta_encode_token, encode_plus, Sector_Roberta, Sector_Roberta_Title_Append_Crf
import torch
...
class Sector_Roberta_Title_Append_Tfidf_Append(Sector_Roberta):
    ...
    def get_ids_and_heads(self, item):
        ids, heads = encode_title_append_topk_tokens_append(self.toker, self.get_tokens(item), self.get_title(item), self.get_top3_tfidf_tokens(item)) # With title, top3 tokens
        return ids, heads


# With CRF
class Sector_Roberta_Title_Append_Crf_Tfidf_Append(Sector_Roberta_Title_Append_Crf):
    def get_title(self, item):
        return item[4] # tokens, labels, is_title, para_first, title, tf_idf
    def get_top3_tfidf_tokens(self, item):
        tfidf_scores = item[5] # tokens, labels, is_title, para_first, title, tf_idf
...
def run_with_crf():
    ...
    for repeat in range(3):
        for split in range(5):
            ...
            testset = flatten(test_set)
            # Train
            m = Sector_Roberta_Title_Append_Crf_Tfidf_Append()
            train_and_save_checkpoints(m, f'ROBERTA_TITLE_APPEND_CRF_TFIDF_RP{repeat}_DS{split}', trainset, devset, testset, check_step = 300, total_step = 3000)
